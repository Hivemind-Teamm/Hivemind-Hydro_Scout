...
export default function SignUpPage() {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    border: `1.5px solid ${inputBorder}`, fontSize: 18, outline: "none",
    width: "100%", boxSizing: "border-box", color: textColor,
    background: inputBg, fontFamily: "inherit",
  };
  const pwInputStyle: React.CSSProperties = { ...inputStyle, padding: "13px 48px 13px 20px" };
  ...
  return (
    <div style={{ display: "flex", height: "100dvh", width: "100vw", overflow: "hidden" }}>
      ...
      {/* Right panel */}
      <div style={{ flex: 1, background: panelBg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 48px", overflowY: "auto" }}>
        <div style={{ width: "100%", maxWidth: 420 }}>
          <h1 style={{ fontSize: 56, fontWeight: 700, margin: 0, lineHeight: 1.1, color: textColor }}>Join Us!</h1>
          <p style={{ fontSize: 26, color: textColor, margin: "8px 0 28px" }}>Create an Account</p>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <button type="submit" disabled={submitting} style={{ marginTop: 8, padding: "16px 20px", borderRadius: 24, border: "2px solid #000", background: submitting ? "#e8b800" : "#FED42E", color: "#000", fontWeight: 700, fontSize: 20, cursor: submitting ? "default" : "pointer", width: "100%", fontFamily: "inherit" }}>
              {submitting ? "Creating account…" : "Create Account"}
            </button>
            <button type="button" onClick={() => router.push("/login")} style={{ padding: "15px 20px", borderRadius: 24, border: `1.5px solid ${textColor}`, background: "transparent", color: textColor, fontWeight: 600, fontSize: 20, cursor: "pointer", width: "100%", fontFamily: "inherit" }}>
              Back to Login
            </button>
